...
* @version		$Id: makedata.php 9835 2010-02-07 13:46:53Z skenow $
*/
...
function make_data(&$dbm, &$cm, $adminname, $adminlogin_name, $adminpass, $adminmail, $language, $adminsalt, $gruops){
    ...
    foreach ($modversion['blocks'] as $func_num => $newblock) {
        if ($fp = fopen('../modules/system/templates/blocks/'.$newblock['template'], 'r')) {
            //if (in_array($newblock['template'], array('system_block_user.html', 'system_block_login.html', 'system_block_mainmenu.html', 'system_block_socialbookmark.html', 'system_block_themes.html', 'system_block_search.html','system_admin_block_warnings.html','system_admin_block_cp.html','system_admin_block_modules.html','system_block_newusers.html','system_block_online.html','system_block_waiting.html','system_block_topusers.html'))) {
						// make all blocks not visible, except for the login block -- freeform solutions
						if($newblock['template'] == 'system_block_login.html') {
                $visible = 1;
            } else {
                $visible = 0;
            }
	...
	// not anymore -- freeform solutions
    ...
    $temp = imcms_encryptPass($adminpass, $adminsalt, $mainSalt);
    $regdate = time();
	...
	$daylightSavings = date("I"); // added by freeform solutions, necessary for correct timezone adjustments
	...
	$i++;
	$p++;
    	$dbm->insert('config', " VALUES ($i, 0, $c, 'server_TZ', '_MD_AM_SERVERTZ', '".((date("Z") / 3600) - $daylightSavings)."', '_MD_AM_SERVERTZDSC', 'timezone', 'float', $p)"); // modified to try and use the real timezone, freeform solutions
	$i++;
	$p++;
    	$dbm->insert('config', " VALUES ($i, 0, $c, 'default_TZ', '_MD_AM_DEFAULTTZ', '".((date("Z") / 3600) - $daylightSavings)."', '_MD_AM_DEFAULTTZDSC', 'timezone', 'float', $p)"); // modified to try and use the real timezone, freeform solutions
	$i++;
	$p++;
	$dbm->insert('config', " VALUES ($i, 0, $c, 'use_ext_date', '_MD_AM_EXT_DATE', '"._EXT_DATE_FUNC."', '_MD_AM_EXT_DATEDSC', 'yesno', 'int', $p)");
 	$i++;
	$p++;
   	$dbm->insert('config', " VALUES ($i, 0, $c, 'theme_set', '_MD_AM_DTHEME', 'impresstheme', '_MD_AM_DTHEMEDSC', 'theme', 'other', $p)"); // changed by freeform solutions - use a different default theme that we have customized
	$i++;
	$p++;
	$dbm->insert('config', " VALUES ($i, 0, $c, 'theme_admin_set', '_MD_AM_ADMIN_DTHEME', 'impresstheme', '_MD_AM_ADMIN_DTHEME_DESC', 'theme_admin', 'other', $p)"); // changed by freeform solutions - use a different default theme that we have customized
	$i++;
	$p++;
	...
	$i++;
	$p++;
    	$dbm->insert('config', " VALUES ($i, 0, $c, 'debug_mode', '_MD_AM_DEBUGMODE', '0', '_MD_AM_DEBUGMODEDSC', 'select', 'int', $p)"); // debug turned off -- freeform solutions
