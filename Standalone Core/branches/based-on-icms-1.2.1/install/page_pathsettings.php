...
* @version		$Id: page_pathsettings.php 9768 2010-01-23 11:51:54Z malanciault $
*/
/**
 *
 */
...
class PathStuffController {
	...
	function PathStuffController() {
		...
		if ( isset( $_SESSION['settings']['TRUST_PATH'] ) ) {
			...
			// @hack p.a: If docroot isn't writable but rootpath/trust_path is, use this one
			// (we assume a WebPI install, with trust_path inside webroot, and Web.config file
			//  used to prevent requests to it)
			if ( is_dir( $this->xoopsRootPath . "/trust_path" ) && is_writable( $this->xoopsRootPath . "/trust_path" ) ) {
				$this->xoopsTrustPath = $this->xoopsRootPath . "/trust_path";
			} else {
