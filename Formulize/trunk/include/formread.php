...
if(isset($GLOBALS['xoopsSecurity']) AND $useToken) { // avoid security check for versions of XOOPS that don't have that feature
...
/*		if($k == 'xoops_upload_file'){ 
	...
	if(isset($up) AND $profileForm !== "new") {
...
if($entry) {
	...
	if(in_array($ele_caption, $prevEntry['captions'])) {
		...
		$value = ($value == "{ID}" AND ($ele_type == "text" OR $ele_type == "textarea")) ? $entry : $value; // for textboxes or textarea boxes with a special value, sub in the id_req
		$value = ($value == "{SEQUENCE}" AND ($ele_type == "text" OR $ele_type == "textarea")) ? formulize_getMaxValue($ele_caption, $id_form) : $value; // handle the special {SEQUENCE} term
	...
	$value = ($value == "{ID}" AND ($ele_type == "text" OR $ele_type == "textarea")) ? $num_id : $value; // for textboxes or textarea boxes with a special value, sub in the id_req
	$value = ($value == "{SEQUENCE}" AND ($ele_type == "text" OR $ele_type == "textarea")) ? formulize_getMaxValue($ele_caption, $id_form) : $value; // handle the special {SEQUENCE} term
...
function blankEntries($submittedcaptions, $prevEntry, $entry, $fid) {
	...
	*/
	...
	foreach($prevEntry['captions'] as $existingCaption2) {
		...
		if(!in_array($existingCaption2, $submittedcaptions) AND !$prevEntry['disabled'][$index]) { // only flag a caption as missing if it is not a disabled field
