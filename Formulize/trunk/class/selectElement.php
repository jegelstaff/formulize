...
class formulizeSelectElement extends formulizeformulize {
...
class formulizeSelectElementHandler extends formulizeElementsHandler {
    ...
    function adminPrepare($element) {
		if (!$element){
			...
			$isLinked = 0;
			$userOptions = array();
			$formlinkScope = array(0=>'all');
		} else {
			$listordd = $ele_value[0] == 1 ? 0 : 1;
			$listordd = $ele_value[8] == 1 ? 2 : $options['listordd'];
			...
			$userOptions = $ele_value[2];
			$formlinkScope = explode(",",$ele_value[3]);
		}
		
		return array(
			'listordd'=>$listordd, 'multiple'=>$multiple,
			'rows'=>$rows, 'isLinked'=>$isLinked,
			'userOptions'=>$userOptions, 'formlinkScope'=>$formlinkScope);
    }
