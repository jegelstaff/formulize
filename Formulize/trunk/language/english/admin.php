...
define("_AM_FORM_ENCRYPT_DESC", "If this is very sensitive information, Formulize can encrypt it before it is stored in the database.  This has no effect on any behaviours in Formulize, it simply changes how the information is stored in the database.  This ensures that anyone who has direct access to the database cannot read the information users have entered.<br><br>Information that is encrypted cannot currently be searched or sorted!  Also, encrypting information does slow down the form a bit.<br><br>The encryption is dependent on the current database password; if you change the database password, Formulize will not be able to read any information that was saved previously!");
