...
function getData($framework, $form, $filter = "", $andor = "AND", $scope = "", $limitStart = "", $limitSize = "", $sortField = "", $sortOrder = "", $forceQuery = false, $mainFormOnly = 0, $includeArchived = false, $dbTableUidField = "", $id_reqsOnly = false, $resultOnly = false, $filterElements = null, $cacheKey = "") { // IDREQS ONLY, only works with the main form!! returns array where keys and values are the id_reqs
    if ($framework == "") {
        $framework = 0;
    } // we want to afirmatively make this a zero and not a null or anything else, for purposes of having consistent cacheKeys
    ...
    if (!$cacheKey) {
        return getDataCached($framework, $form, $filter, $andor, $scope, $limitStart, $limitSize, $sortField, $sortOrder, $forceQuery, $mainFormOnly, $includeArchived, $dbTableUidField, $id_reqsOnly, $resultOnly, $filterElements);
    }
                ...
                if ($formFieldFilterMap[$mappedForm][$element_id]['hasother']) {
                    ...
                    if ($ifParts[1] === '' OR $operator == ' IS NULL ' OR $operator == ' IS NOT NULL ') {
                        ...
                        if ($ifParts[1] == "PERGROUPFILTER") {
                            ...
                            // Neal's suggestion:  use EXISTS...other forms of subquery using field IN subquery or subquery LIKE field, and a CONCAT 
                            // in the subquery, failed in various conditions.  IN did not work with multiple selection boxes, and LIKE did not work with 
                            // search terms too general to return only one match in the source form.  Exists works in all cases.  :-)
                            ...
                            if ($ele_value[1]) {
                                ...
                                $newWhereClause = " EXISTS (SELECT 1 FROM " . DBPRE . "formulize_" . $sourceFormObject->getVar('form_handle') . " AS source WHERE $queryElement = source.entry_id AND source.`" . $sourceMeta[1] . "`" . $operator . $quotes . $likebits . mysql_real_escape_string($ifParts[1]) . $likebits . $quotes . ")";
                            }
    ...
    if (is_array($entry)) {
        foreach ($entry as $formHandle => $record) {
            foreach ($record as $elements) {
                if (array_key_exists($handle, $elements)) {
                    return $formHandle;
                }
            }
        }
