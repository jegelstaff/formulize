...
function convertAllHandlesAndIds($handles, $frid, $reverse = false, $ids = false, $fid = false) {
    ...
    if (!isset($cachedElementHandles[$frid]) OR ($fid AND !isset($cachedElementHandlesFromElementIds[$fid]))) {
        ...
        if (in_array("creation_uid", $handles)) {
            $to_return[] = "creation_uid";
        }
        if (in_array("uid", $handles)) {
            $to_return[] = "creation_uid";
        }
        if (in_array("creation_datetime", $handles)) {
            $to_return[] = "creation_datetime";
        }
        if (in_array("creation_date", $handles)) {
            $to_return[] = "creation_datetime";
        }
        if (in_array("mod_uid", $handles)) {
            $to_return[] = "mod_uid";
        }
        if (in_array("proxyid", $handles)) {
            $to_return[] = "mod_uid";
        }
        if (in_array("mod_date", $handles)) {
            $to_return[] = "mod_datetime";
        }
        if (in_array("mod_datetime", $handles)) {
            $to_return[] = "mod_datetime";
        }
        if (in_array("creator_email", $handles)) {
            $to_return[] = "creator_email";
        }
 ...
 *  @author Gordon Woodmansey (edited by Sybil Ehrensberger)
 *  @param  $templatename A Formulize template name
 *  @param  $templateVariables The template Variables
 *  @param  $sid The screen id beloning to the template
 */

function renderTemplate($templatename, $templateVariables, $sid) {
