...
if($viewentry AND !$isadmin) // allow admins to view entries, but check for perms for co-membership in a group for other users
{
...
//				print "thisterm is now: $thisterm<br><br>";
			...
			if($globalandor == "and") // if it's an and do the interim query thing, otherwise, behave normally.
			{
				...
				if(!$savedIds[0] AND $andorArray[$i] == "and") // set the globalAndPrepQ to empty if the local and/or is AND and there is a term that returns nothing, then break out of the foreach so no further terms are processed.
				{
					$globalAndPrepQ = "";
					break 2;			
				}
				...
				foreach($savedIds as $onemasid) // build the query that will be used to get the id_reqs for comparison after we're done all the local terms for this field
				{
					...
					{
						$globalAndPrepQ .= " OR ele_id" . $gandeq . $onemasid;
					}
				}
		...
		else // we're doing a global AND
		{
				...
				$master_reqsq = "SELECT id_req FROM " . $xoopsDB->prefix("form_form") . " WHERE $globalAndPrepQ";
	...
	foreach( $id as $i ){
		...
		if( !empty($ele[$i]) ){
			...
			$msg.= "<table border=1 bordercolordark=black bordercolorlight=#C0C0C0 width=500><td><b>".$ele_caption."</b><br>";
					...
					$value = trim($ele[$i]); // trim added by jwe 9/01/04
				break;
				case 'textarea':
			$msg.= "<table border=1 bordercolordark=black bordercolorlight=#C0C0C0 width=500><td><b>".$ele_caption."</b><br>";
					$msg.= $myts->stripSlashesGPC($ele[$i])."<br></td></table><br>";
