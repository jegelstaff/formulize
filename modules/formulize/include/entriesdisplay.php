...
function displayEntries($formframe, $mainform="", $loadview="", $loadOnlyView=0, $viewallforms=0, $screen=null) {
  ...
  formulize_benchmark("start of drawing list");
	...
	if($_POST['saveid_formulize'] AND $formulize_LOESecurityPassed AND ($update_other_reports OR ((is_numeric($_POST['saveid_formulize']) AND ($update_own_reports AND $xoopsUser->getVar('uid') == getSavedViewOwner($_POST['saveid_formulize']))) OR $_POST['saveid_formulize'] == "new"))) {
		...
		$savename = $xoopsDB->escape($savename);
		$savesearches = $xoopsDB->escape($_POST['asearch']);
		//print $_POST['asearch'] . "<br>";
		//print "$savesearches<br>";
		$qsearches = $xoopsDB->escape($qsearches);
		...
		default: // only thing left is custom
			...
			foreach($setting as $thisSetting) {
				...
				if(substr($thisSetting,0,1)=="!") {
  				$allowed[] = $xoopsDB->escape(substr($thisSetting,1));
				} else {
					$excluded[] = $xoopsDB->escape($thisSetting);
				}
			}
			break;
	}
...
function loadReport($id, $fid, $frid) {
  ...
  if(is_numeric($id)) {
    ...
    $thisview = q("SELECT * FROM " . $xoopsDB->prefix("formulize_saved_views") . " WHERE sv_name='".$xoopsDB->escape($id)."' AND sv_formframe = $formframe AND sv_mainform = $mainform");
  }
      ...
      $addToORFilter = false; // flag to indicate if we need to apply the current search term to a set of "OR'd" terms			
        ...
        if($operator == "!") { $operator = "NOT LIKE"; }
			...
			if($addToItsOwnORFilter) {
				...
				if(!$ORstart) { $ORfilter .= "]["; }
				$ORfilter .= $key . "/**/$one_search"; // . $xoopsDB->escape($one_search); // mysql_real_escape_string no longer necessary here since the extraction layer does the necessary dirty work for us
				$ORstart = 0;
			} else {
				if(!$start) { $filter .= "]["; }
				$filter .= $key . "/**/$one_search"; // . $xoopsDB->escape($one_search); // mysql_real_escape_string no longer necessary here since the extraction layer does the necessary dirty work for us
				$start = 0;
			}
