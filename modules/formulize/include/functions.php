 ...
 * @param number $fid The form id
 * @param object $form_handler Optional xoops module form handler
 * @return string The title of the form
 */
// get the title of a form
function getFormTitle($fid, $form_handler = null) {
    $form_handler = $form_handler ? $form_handler : xoops_getmodulehandler('forms', 'formulize');
...
/**
 * Get the current URL
 * @return string The current URL of the request
 */
...
/**
 * Get a human readable comma separated list of group names
 * @param string $list A comma separated list of group ids
 * @param boolean $obeyMemberOnlyFlag @todo Get a proper description of this param
 * @return string A human readable comma separated list of group names
 */
...
/**
 * Returns the owner of a given saved view
 * only checks based on 2.0 saved view format, not 1.6 or earlier format
 *
 * @param number $vid The id of the saved view.
 * @return number The user ID of the saved form owner.
 */
...
/**
 * Fetch and return the results of an raw SQL statement
 *
 * Returns a multidimensioned array where the first index is the row of the result
 * and the second index is the field name in that row. Borrowed from the extraction layer,
 * but modified to use the XOOPS DB class.
 *
 * @param string $query The SQL query to execute
 * @param string $keyfield An optional keyfield to set the key of the result array to be one of the fields in the query. Useful if you want to use isset with a value to determine the presence of something in the result set, instead of searching the array
 * @param boolean $keyfieldOnly A flag to indicate if only the keyfield should be returned (Only used when keyfield is present)
 * @return array The returned array of query results
 */
    ...
    switch($thisDefaultEle->getVar('ele_type')) {
      ...
      case "date":
        ...
        if (false !== $defaultTextToWrite) {
            $defaultTextToWrite = is_int($defaultTextToWrite) ? date("Y-m-d", $defaultTextToWrite) : $defaultTextToWrite;
        }
        break;
      case "radio":
