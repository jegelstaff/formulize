 ...
 * @param string $targetAlias - the alias used in SQL to refer to the table that the conditions should apply to.  This is optional in general, but required if the query you're building for uses an alias!
 * @return array - returns an array with three elements: 0 - the 'match all' SQL suitable for the WHERE clause, 1 - the 'match one or more' SQL suitable for the WHERE clause, 2 - the join clause necessary for including the form that { } references are pointing to
 */
...
/**
 * Find the forms - this element's own form, plus any form connected to it through the primary relationship - that
 * contain derived value elements whose formula references the given element. A formula "references" the element if
 * it contains the element's handle written as $handle, 'handle' or "handle". Used to offer a targeted, user-triggered
 * "recompute derived values" step after an element's default value has been applied to existing entries (recomputing
 * can be expensive, so it is kept separate from the apply operation and batched, like the derived value element's own
 * "update all entries" feature).
 *
 * @param object $element The element whose handle we are looking for references to
 * @return array Map of form id => form title, for the forms that have at least one dependent derived value element
 */
function formulize_getFormsWithDependentDerivedValues($element) {
    ...
    // candidate forms: this element's form, plus the forms directly connected to it in the primary relationship (frid -1).
    // getLinksGroupedByForm, scoped to this form, returns only the links that touch it, so each link's two sides are
    // this form and a form connected to it.
    $thisFid = intval($element->getVar('id_form'));
    $candidateFids = array($thisFid => true);
    if($primaryRelationship = $framework_handler->get(-1)) {
        foreach($framework_handler->getLinksGroupedByForm($primaryRelationship, $thisFid) as $formLinks) {
            foreach($formLinks as $link) {
                $candidateFids[intval($link['form1'])] = true;
                $candidateFids[intval($link['form2'])] = true;
            }
        }
    }
    $candidateFids = array_keys($candidateFids);
    if(empty($candidateFids)) { return array(); }

    // a derived formula references the element if it contains $handle (not part of a longer name), 'handle' or "handle"
    $quotedHandle = preg_quote($handle, '/');
    $referencePattern = '/\$' . $quotedHandle . '(?![A-Za-z0-9_])|\'' . $quotedHandle . '\'|"' . $quotedHandle . '"/';

    $dependentForms = array();
    $derivedSql = "SELECT ele_id, id_form FROM ".$xoopsDB->prefix("formulize")." WHERE id_form IN (".implode(',', array_map('intval', $candidateFids)).") AND ele_type='derived'";
    if($derivedRes = $xoopsDB->query($derivedSql)) {
        while($derivedRow = $xoopsDB->fetchArray($derivedRes)) {
            if(isset($dependentForms[$derivedRow['id_form']])) { continue; } // this form already qualifies
            // the derived element is instantiated as a formulizeDerivedElement, whose getVar('ele_value') transparently
            // loads the formula (which lives in a code/derived_<handle>.php file) into element [0] for us
            $derivedElement = $element_handler->get($derivedRow['ele_id']);
            if(!is_object($derivedElement)) { continue; }
            $ele_value = $derivedElement->getVar('ele_value');
            $formula = isset($ele_value[0]) ? strval($ele_value[0]) : '';
            if($formula AND preg_match($referencePattern, $formula)) {
                $formObject = $form_handler->get($derivedRow['id_form']);
                $dependentForms[$derivedRow['id_form']] = is_object($formObject) ? $formObject->getVar('title') : $derivedRow['id_form'];
            }
        }
    }
