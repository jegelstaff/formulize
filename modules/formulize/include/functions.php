...
function getFormFramework($formframe, $mainform="") {
    ...
    if (!empty($mainform)) {
        ...
        if (!is_numeric($formframe)) {
            $frameid = q("SELECT frame_id FROM " . $xoopsDB->prefix("formulize_frameworks") . " WHERE frame_name='" . $xoopsDB->escape($formframe) . "'");
        ...
        if (!is_numeric($mainform)) {
            ...
            if ($GLOBALS['formulize_versionFourOrHigher'] == false) {
                $formcheck = q("SELECT ff_form_id FROM " . $xoopsDB->prefix("formulize_framework_forms") . " WHERE ff_frame_id='$frid' AND ff_handle='" . $xoopsDB->escape($mainform) . "'");
        ...
        if (!is_numeric($formframe)) { // if it's a title, convert to the id
            $formid = q("SELECT id_form FROM " . $xoopsDB->prefix("formulize_id") . " WHERE desc_form = '" . $xoopsDB->escape($formframe) . "'");
    ...
    if (isset($GLOBALS['formulize_other']) and is_array($GLOBALS['formulize_other'])) {
        foreach ($GLOBALS['formulize_other'] as $ele_id=>$value) {
            ...
            if ($value != "" AND $existing_value) {
                // update
                $sql = "UPDATE " . $xoopsDB->prefix("formulize_other") . " SET other_text=\"" . $xoopsDB->escape($value) . "\" WHERE id_req='$id_req' AND ele_id='$ele_id'";
            }elseif ($value != "" AND !$existing_value) {
                // add
                $sql = "INSERT INTO " . $xoopsDB->prefix("formulize_other") . " (id_req, ele_id, other_text) VALUES (\"$id_req\", \"$ele_id\", \"" . $xoopsDB->escape($value) . "\")";
            }elseif ($value == "" AND $existing_value) {
                // delete
...
function formatLinks($matchtext, $handle, $textWidth=35, $entryBeingFormatted) {
    ...
    if (($ele_value[4] > 0 AND $ele_type=='text') OR ($ele_value[3] > 0 AND $ele_type=='textarea')) {
        ...
        if (isset($cachedQueryResults[$boxproperties[0]][$boxproperties[1]][$entryBeingFormatted][$handle])) {
            ...
            $element_id_q = q("SELECT ele_id FROM " . $xoopsDB->prefix("formulize") . " WHERE id_form='" . $boxproperties[0] . "' AND ele_handle='" . $xoopsDB->escape($boxproperties[1]) . "' LIMIT 0,1");
            $formulize_mgr = xoops_getmodulehandler('elements', 'formulize');
        ...
        if (isset($cachedUidResults[$matchtext])) {
            ...
            $uids = q("SELECT uid FROM " . $xoopsDB->prefix("users") . " WHERE $nametype = '" . $xoopsDB->escape($matchtext) . "' $archiveFilter");
...
function sendNotifications($fid, $event, $entries, $mid="", $groups=array()) {
    ...
    $cons = q("SELECT * FROM " . $xoopsDB->prefix("formulize_notification_conditions") . " WHERE not_cons_fid=".intval($fid)." AND not_cons_event=\"".$xoopsDB->escape($event)."\"");
    if (count($cons) == 0) {
        return;
    }
...
function compileNotUsers2($uids_conditions, $uids_complete, $notification_handler, $fid, $event, $mid) {
    ...
    $subd_uids = q("SELECT not_uid FROM " . $xoopsDB->prefix("xoopsnotifications") . " WHERE not_event=\"".$xoopsDB->escape($event)."\" AND not_category=\"form\" AND not_modid=$mid AND not_itemid=$fid");
    $uids_subd = array();
...
function writeElementValue($formframe = "", $ele, $entry, $value, $append, $prevValue=null, $lvoverride=false, $linkedTargetHint = "") {
    ...
    if ($foundit = strstr($ele_value[2], "#*=:*") AND !$lvoverride) {
        ...
        foreach ($value as $thisValue) {
            if (isset($cachedEntryIds[$boxproperties[0]][$boxproperties[1]][$thisValue])) {
                ...
                $searchForValues[] = $xoopsDB->escape(html_entity_decode($thisValue));
            }
        }
    ...
    if ($entry == "new") {
        ...
        $sql="INSERT INTO ".$xoopsDB->prefix("formulize_".$elementFormObject->getVar('form_handle'))." (creation_datetime, mod_datetime, creation_uid, mod_uid, `".$element->getVar('ele_handle')."`) VALUES (NOW(), NOW(), \"$owner\", \"$uid\", '".$xoopsDB->escape($value)."')";
        $needToSetOwner = true;
    } else {
        ...
        $sql = "UPDATE ".$xoopsDB->prefix("formulize_".$elementFormObject->getVar('form_handle'))." SET `".$element->getVar('ele_handle')."` = '".$xoopsDB->escape($valueToWrite)."' WHERE entry_id=".intval($entry);
    }
...
function dealWithDeprecatedFormHandles($handle) {
    ...
    if (!isset($cachedHandles[$handle])) {
        $sql = "SELECT ff_form_id FROM ".$xoopsDB->prefix("formulize_framework_forms")." WHERE ff_handle = '".$xoopsDB->escape($handle)."'";
        if ($res = $xoopsDB->query($sql)) {
    ...
    if ($subfilter AND !(isset($_POST[$linked_data_id])) AND !(isset($_GET[$linked_data_id]))) {
        ...
        if (strstr($options, "#*=:*")) {
            ...
            if (is_array($limit)) {
                ...
                $limitCondition = " WHERE t1`".$limit['ele_id']."` ".$limitOperator." '$likebits".$xoopsDB->escape($limit['term'])."$likebits' ";
                ...
                $limitCondition = ", ".$xoopsDB->prefix("formulize_".$linkedFormObject->getVar('form_handle'))." as t2 WHERE t1.`$linked_ele_id` LIKE CONCAT('%',t2.entry_id,'%') AND t2.`".$linkedSourceElementEleValueParts[1]."` LIKE '%".$xoopsDB->escape($_POST[$linked_data_id])."%'";
            }
            unset($options);
        ...
        for ($filterId = 0;$filterId<count($filterElementHandles);$filterId++) {
            ...
            if (substr($filterTerms[$filterId],0,1) == "{" AND substr($filterTerms[$filterId],-1)=="}") {
                ...
                if (isset($_GET[$bracketlessFilterTerm])) {
                    $filterTerms[$filterId] = $xoopsDB->escape($_GET[$bracketlessFilterTerm]);
                }
            }
...
function _buildConditionsFilterSQL($filterId, &$filterOps, &$filterTerms, $filterElementIds, $targetFormElementTypes, $curlyBracketEntry, $userComparisonId, $curlyBracketForm, $element_handler, $form_handler) {
        ...
        if ($targetElementObject->isLinked) {
            ...
            if (substr($filterTerms[$filterId],0,1) == "{" AND substr($filterTerms[$filterId],-1)=="}") {
                if (isset($GLOBALS['formulize_asynchronousFormDataInDatabaseReadyFormat'][$curlyBracketEntry][substr($filterTerms[$filterId],1,-1)])) {
                    ...
                    $filterTermToUse = " curlybracketform.`".$xoopsDB->escape(substr($filterTerms[$filterId],1,-1))."` ";
                ...
                $filterTermToUse = $xoopsDB->escape($filterTerms[$filterId]);
            }
    ...
    if (!$conditionsFilterComparisonValue) {
        $conditionsFilterComparisonValue = $quotes.$likebits.$xoopsDB->escape($filterTerms[$filterId]).$likebits.$quotes;
    }
    ...
    if (substr($filterTerms[$filterId],0,1) == "{" AND substr($filterTerms[$filterId],-1)=="}" AND !$targetElementObject->isLinked) {
        if (isset($GLOBALS['formulize_asynchronousFormDataInDatabaseReadyFormat'][$curlyBracketEntry][substr($filterTerms[$filterId],1,-1)])) {
            $conditionsFilterComparisonValue = "'".$xoopsDB->escape($GLOBALS['formulize_asynchronousFormDataInDatabaseReadyFormat'][$curlyBracketEntry][substr($filterTerms[$filterId],1,-1)])."'";
