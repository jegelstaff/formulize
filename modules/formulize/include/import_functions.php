			...
			if(!is_array($id_reqs)) { // if we're dealing with a blank template...
	            ...
	            for($element = 0; $element < $elements; $element++)
	            {
					...
					if(strstr($options, "#*=:*")) {
					    ...
					    " AND ele_handle='" . $xoopsDB->escape($parts[1]) . "'";
					...
					case "radio":
						...
						if($this_id_req) { // updating an entry
							...
							foreach($fieldValues as $elementHandle=>$fieldValue) {
								...
								$start = false;
								$updateSQL .= "`$elementHandle` = '".$xoopsDB->escape($fieldValue)."'";
							}
							...
							foreach($fieldValues as $elementHandle=>$fieldValue) {
									...
									$values .= ", '".$xoopsDB->escape($fieldValue) . "'";
...
function getElementID($id_form, $ele_caption, $ele_value)
{
	...
	if(!isset($cachedElementIDs[$id_form][$ele_caption][$ele_value])) {
		...
		" AND ele_caption='" . $xoopsDB->escape(formformCaption($ele_caption)) . "'" .
		" AND ele_value='" . $xoopsDB->escape($myts->htmlSpecialChars($ele_value)) . "'";
...
function getUserID($stringName)
{
    ...
    $sql = "SELECT uid FROM " . $xoopsDB->prefix("users") .  
        " WHERE uname='" . $xoopsDB->escape($stringName) . "'";
    ...
    {
	    $sql = "SELECT uid FROM " . $xoopsDB->prefix("users") .  
        " WHERE name='" . $xoopsDB->escape($stringName) . "'";
