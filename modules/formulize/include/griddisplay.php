...
function catalogueGridElement($elementId, $entry_id, $containingGridElementIdOrObject, $formObjectPreppedForRender, $prevEntry = array(), $screen = null) {
	...
	if(isset($gridDisplayConditions[0]) AND is_array($gridDisplayConditions[0]) AND count($gridDisplayConditions[0]) > 0 ) {
		catalogConditionalElement($elementInGridMarkupName, array_unique($gridDisplayConditions[0]), $screen);
	...
	if(isset($elementInGridDisabledConditions[0]) AND is_array($elementInGridDisabledConditions[0]) AND count($elementInGridDisabledConditions[0]) > 0 ) {
		catalogConditionalElement($containerElementMarkupName, array_unique($elementInGridDisabledConditions[0]), $screen);
	}
}
...
function elementsInGrid($startID, $fid, $gridCount = 0, $requiredOnly = false) {
	...
	if(!isset($cachedGridElements[$fid][$startID][$gridCount][$requiredOnly])) {
		...
		$element_ids_result = q("SELECT ele_id FROM " . $xoopsDB->prefix("formulize") . " WHERE $required ele_order >= '$starting_order' AND id_form='$fid' AND ele_id IN (".implode(",",$formObject->getVar('elements')).") AND ele_type != 'grid' ORDER BY ele_order $limitCondition", 'ele_id', true);
