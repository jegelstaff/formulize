...
define("_AM_EMBED_SESSION_DEPENDS_BODY", "The session cookie for this site is set to <code>SameSite=%s</code>, under Sessions &amp; cookies. Browsers send it into a frame only when the page doing the framing is on the same domain as this site (%s) and uses the same https or http. Embedded there, a screen shows the visitor their own data, exactly as it would here. Embedded on any other domain, no session reaches it: the screen is anonymous however the visitor is signed in, it can only show what the Anonymous group is allowed to see, and a screen anonymous visitors cannot use will appear empty.");
define("_AM_EMBED_SESSION_LIST_SAME", "<b>These share this site's session</b>, so screens embedded there are signed in:");
define("_AM_EMBED_SESSION_LIST_CROSS", "<b>These are a different site to the browser</b>, so screens embedded there are anonymous:");
define("_AM_EMBED_SESSION_DEPENDS_LMS", "If you are embedding in a system on another domain and your users need to see their own data there, such as a learning management system, change the SameSite setting to <i>None</i>. Read the note that appears here when you do before you decide.");
