...
define("_AM_SCREEN_SETTINGS_EMBED_CODE_DESC", "Paste this into the page that is to display the screen. Include the script once per page, however many screens that page shows. Each screen stays as tall as its content, so there is no height to guess at, and as wide as the page gives it, but never narrower than 400 pixels, so it cannot end up too narrow to use.");
define("_AM_SCREEN_SETTINGS_EMBED_CODE_ADDRESS_DESC", "Replace <code>%1\$s</code> in both places. It can be the URL for this site, <code>%2\$s</code>, if you only need anonymous users to access the screen. For supporting logged in users, see the <a href='https://formulize.org/documentation/embedding_screens/' target='_blank'>embedding screens documentation on formulize.org</a>.");
define("_AM_SCREEN_SETTINGS_CODE_COPY", "Copy");
define("_AM_SCREEN_SETTINGS_CODE_COPIED", "Copied");
