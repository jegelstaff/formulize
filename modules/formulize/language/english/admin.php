...
define("_AM_SCREEN_SETTINGS_EMBED_CODE_DESC", "Paste this into the page that is to display the screen. Include the script once per page, however many screens that page shows. It keeps each screen as tall as its content, so there is no height to guess at.<br><br>Change the address in the code to one on the same domain as the page you are pasting it into, and set that address up on this server, or visitors will not be signed in and will not be able to submit the screen: <a href='https://formulize.org/developers/embedding_screens_setup/' target='_blank'>formulize.org/developers/embedding_screens_setup</a>");
...
define("_AM_EMBED_SESSION_DEPENDS_BODY", "The session cookie for this site is set to <code>SameSite=%s</code>, under Sessions &amp; cookies. Browsers send it into a frame only when the page doing the framing is on the same domain as this site (%s) and uses the same https or http. Embedded there, a screen shows the visitor their own data and saves normally, exactly as it would here. Embedded on any other domain, no cookies reach it at all: the screen is anonymous however the visitor is signed in, it can only show what the Anonymous group is allowed to see, <b>submitting fails</b>, a passcode screen asks for the passcode again on the next page, and an anonymous visitor cannot get back to an entry they saved earlier. Setting up an address on the same domain: <a href='https://formulize.org/developers/embedding_screens_setup/' target='_blank'>formulize.org/developers/embedding_screens_setup</a>");
...
define("_AM_EMBED_SESSION_LIST_CROSS", "<b>These are a different site to the browser</b>, so screens embedded there are anonymous and cannot be submitted:");
