...
define("_AM_ELE_DERIVED_DESC", "Beispiele:<br />\$value = \$profile_first_name . ' ' . \$profile_last_name; // für einen vollständigen Namen<br>\$value = \$inventory_value * \$inventory_count; // für einen Gesamtwert<br><br>Die Formel wird in PHP-Code geschrieben und das Element verwendet den Wert, der in <b>\$value</b> im Code zugewiesen wird. Als PHP-Code können Sie alles in der <a href='https://formulize.org/developers/API/' target='_blank'>Formulize API</a> verwenden, aber für komplexe Operationen werden die <b>Vor dem Speichern</b> und <b>Nach dem Speichern Verfahren</b> anstelle von abgeleiteten Werten empfohlen.<br><br>Verfügbare Variablen sind:<br><b>\$form_id</b> - die ID-Nummer des Hauptformulars im aktuellen Datensatz, normalerweise das Formular, zu dem der aktive Bildschirm gehört<br><b>\$entry_id</b> - die Eintrags-ID des Datensatzes in der Datenbank, zu dem dieser Wert in diesem bestimmten Formular gehört<br><b>\$relationship_id</b> - die ID-Nummer der aktiven Beziehung, falls vorhanden. Standardmäßig ist die primäre Beziehung die aktive Beziehung, es sei denn, der aktive Bildschirm ist speziell für etwas anderes festgelegt.<br><b>\$entry</b> - der Eintrag, zu dem dieser Wert gehört, wie durch die gatherDataset-Funktion aus dem aktuellen Datensatz erfasst. Geeignet zum Übergeben an die Anzeigefunction, aber es wird empfohlen, stattdessen auf Elemente durch ihre Handle-Variablen zu verweisen:<br><br>Sie können jeden Wert aus dem aktuellen Datensatz abrufen, indem Sie auf Element-Handles als Variablen verweisen, z.B.: \$activities_name. Dies gibt Ihnen den lesbaren Wert dieses Elements im aktuellen Datensatz. Beachten Sie, dass dasselbe Element-Handle je nach Art der Aufnahme dieses Formulars in den aktuellen Datensatz bei der Speicherung des Eintrags unterschiedliche Dinge zurückgeben kann. Sie könnten beispielsweise auf ein Element in einem anderen Formular verweisen, und dieses Formular ist in einigen Kontexten das Hauptformular des Datensatzes, aber manchmal wird dieses Formular als Unterformular verwendet. Der abgeleitete Wert erhält zu verschiedenen Zeiten unterschiedliche Daten, wenn er unter unterschiedlichen Umständen berechnet wird. Wenn sich mehrere Einträge aus einem Formular im Datensatz befinden, geben Element-Handles auf diesem Formular ein Array von Werten aus allen Einträgen zurück.");
