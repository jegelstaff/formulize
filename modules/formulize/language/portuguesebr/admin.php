...
define("_AM_ELE_DERIVED_DESC", "Exemplos:<br />\$value = \$profile_first_name . ' ' . \$profile_last_name; // para um Nome Completo<br>\$value = \$inventory_value * \$inventory_count; // para um Valor Total<br><br>A fórmula é escrita em código PHP, e o elemento usará o valor atribuído a <b>\$value</b> no código. Como código PHP, você pode usar tudo na <a href='https://formulize.org/developers/API/' target='_blank'>API Formulize</a>, mas para operações complexes, os <b>Procedimentos Antes de Salvar</b> e <b>Depois de Salvar</b> são recomendados em vez de valores derivados.<br><br>As variáveis disponíveis são:<br><b>\$form_id</b> - o número de identificação do formulário principal no conjunto de dados atual, geralmente o formulário ao qual a tela ativa pertence<br><b>\$entry_id</b> - a ID da entrada do registro no banco de dados ao qual este valor pertence neste formulário específico<br><b>\$relationship_id</b> - o número de identificação da relação ativa, se houver. Por padrão, a Relação Primária é a relação ativa, a menos que a tela ativa seja especificamente configurada para usar outra coisa.<br><b>\$entry</b> - a entrada à qual este valor pertence, conforme coletado pela função gatherDataset do conjunto de dados ativo. Adequado para passar para a função de exibição, mas é recomendado fazer referência aos elementos por suas variáveis de identificador:<br><br>Você pode coletar qualquer valor do conjunto de dados ativo, referindo-se aos identificadores de elementos como variáveis, por exemplo: \$activities_name. Isso lhe dará o valor legível por humanos desse elemento no conjunto de dados atualmente ativo. Observe que o mesmo identificador de elemento pode retornar coisas diferentes dependendo de como este formulário é incluído no conjunto de dados atual quando a entrada é salva. Por exemplo, você pode fazer referência a um elemento em outro formulário, e esse formulário é o formulário principal do conjunto de dados em alguns contextos, mas outras vezes esse formulário é usado como um subformulário. O valor derivado receberá dados diferentes em momentos diferentes, se for calculado em circunstâncias diferentes. Quando há várias entradas de um formulário no conjunto de dados, os identificadores de elementos nesse formulário retornarão uma matriz de valores de todas as entradas.");
