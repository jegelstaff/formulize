...
class formulizePermHandler  {
	...
	function _getGroupsHavingScopeInfo($gids, $different=false) {
		...
		if(!$different) {
			...
			$sql = "SELECT groupid FROM ".$xoopsDB->prefix("formulize_groupscope_settings")." as t1 WHERE fid = ".$this->fid." AND view_groupid != 0 AND NOT EXISTS(SELECT 1 FROM ".$xoopsDB->prefix("formulize_groupscope_settings")." as t2 WHERE view_groupid IN (".mysql_real_escape_string(implode(", ", $gids)).") AND fid=".$this->fid." AND t1.groupid = t2.groupid)";
		}
