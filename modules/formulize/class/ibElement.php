...
// Insert Break (IB) element - Text for display spanning the form

require_once XOOPS_ROOT_PATH . "/modules/formulize/class/elements.php";
require_once XOOPS_ROOT_PATH . "/modules/formulize/include/functions.php";

class formulizeIbElement extends formulizeElement {

	public static $category = "layout";

	function __construct() {
		$this->name = "Text for display (spanning the form)";
		$this->hasData = false; // IB is a non-data element
...
class formulizeIbElementHandler extends formulizeElementsHandler {
	...
	function create() {
		return new formulizeIbElement();
	}

	function getDefaultEleValue() {
		$ele_value[0] = '';
		$ele_value[1] = ''; // class for the table cell when form is table rendered
		return $ele_value;
	}
	...
	/**
	 * Renders the IB element (insert break / text for display spanning the form)
	 * Returns an array instead of an XoopsForm object - item 0 is content, item 1 is CSS class
	 */
	function render($ele_value, $caption, $markupName, $isDisabled, $element, $entry_id, $screen=false, $owner=null) {
		// If ele_value[0] is empty, use caption instead
		if(trim($ele_value[0]) == "") { 
			$ele_value[0] = $caption; 
		}
		...
		if(strstr($ele_value[0], "\$value=") OR strstr($ele_value[0], "\$value =")) {
			...
			if($evalResult === false) {
				...
				$ele_value[0] = $elementRenderer->formulize_replaceReferencesAndVariables(
					...
					$markupName, 
					$screen
				); // in case PHP code generated some { } references
			}
		}
		
		// Return an array, not a form object - item 0 is contents, item 1 is CSS class
		return $ele_value;
	}

	function getDefaultValue($element, $entry_id = 'new') {
		return null; // IB elements don't have values
	}

	function loadValue($element, $value, $entry_id) {
		return $element->getVar('ele_value'); // IB doesn't load values from database
	}

	function prepareDataForSaving($value, $element, $entry_id=null, $subformBlankCounter=null) {
		return null; // IB elements don't save data
	}

	function generateValidationCode($caption, $markupName, $element, $entry_id=false) {
		return array(); // No validation needed for IB elements
	}

	function prepareDataForDataset($value, $handle, $entry_id) {
		return null; // IB elements don't have data in datasets
	}

	function prepareLiteralTextForDB($value, $element, $partialMatch=false) {
		return null; // IB elements don't interact with database values
	}

	function formatDataForList($value, $handle="", $entry_id=0, $textWidth=100) {
		return ""; // IB elements don't appear in lists
	}
}
