...
class formulizeColorpickElement extends formulizeElement {
	...
	function __construct() {
		...
		$this->overrideDataType = "varchar(7)"; // use this to set a datatype for the database if you need the element to always have one (like 'date').  set needsDataType to false if you use this.
...
class formulizeColorpickElementHandler extends formulizeElementsHandler {
	...
	function create() {
		return new formulizeColorpickElement();
	}
	...
	function adminPrepare($element) {
		...
		if(is_object($element) AND is_subclass_of($element, 'formulizeElement')) {
			...
			$dataToSendToTemplate['ele_value'] = array("#FFFFFF");
		}
	...
	function getDefaultValue($element, $entry_id = 'new') {
		...
		return isset($ele_value[0]) ? $ele_value[0] : "#FFFFFF";
	}
