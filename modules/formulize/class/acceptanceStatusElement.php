...
class formulizeAcceptanceStatusElement extends formulizeformulize {
    
    var $labels; 
    
    function __construct() {
        $this->name = "Acceptance Status";
        ...
        $this->overrideDataType = "json"; // use this to set a datatype for the database if you need the element to always have one (like 'date').  set needsDataType to false if you use this.
...
class formulizeAcceptanceStatusElementHandler extends formulizeElementsHandler {
    ...
    function create() {
        return new formulizeAcceptanceStatusElement();
    }
    ...
    function render($ele_value, $caption, $markupName, $isDisabled, $element, $entry_id, $screen, $owner) {
        // get the courses the instructor is teaching this year
        // show a dropdown for each course
        global $xoopsDB;
        ...
        $assignedSections = getData(15, 4, 'instr_assignments_instructor/**/'.$instructorEntryId.'/**/=][ro_module_year/**/'.$row[1].'/**/=');
        $assignmentData = json_decode($ele_value); // turns it into an object
        $output = "";
    ...
    function prepareDataForSaving($value, $element, $entry_id=null, $subformBlankCounter=null) {
        return json_encode($value); // strictly speaking, formulize will already escape all values it writes to the database, but it's always a good habit to never trust what the user is sending you!
    }
    ...
    function prepareDataForDataset($value, $handle, $entry_id) {
        $assignmentData = json_decode($value); // will be an object
        $assignmentData = get_object_vars($assignmentData); // basically convert to an array, returns key value pairs of the properties and their values
        ...
        $sql = 'SELECT s.entry_id as id, CONCAT(ro_module_course_code, "-", sections_section_number) as label FROM '.$xoopsDB->prefix('formulize_course_components').' as s
            ...
            WHERE s.entry_id IN ('.implode(',',array_keys($assignmentData)).')';
        $res = $xoopsDB->query($sql);
        $labels = array();
