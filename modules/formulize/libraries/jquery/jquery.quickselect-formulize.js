...
(function($){
    ...
    QuickSelect = function($input_element, options){
        ...
        var moveSelect = function(step_or_li){
            ...
            if (!lis)
                return;
            if (typeof(step_or_li) === "number")
                activeSelection = activeSelection + step_or_li;
            else
                activeSelection = lis.index(step_or_li);
            ...
            if (options.autoFill && self.last_keyCode != 8) {
                // autoFill value, if option is set and the last user key pressed wasn't backspace
        ...
        var selectCurrent = function() {
            ...
            var repopulate_items = function(items) {
                ...
                if (!hasFocus || items === null || items.length === 0) {
            ...
            var show_results = function(){
                ...
                if (options.autoSelectFirst || (options.selectSingleMatch && $lis.length == 1))
                    moveSelect($lis.get(0));
            };
            var onChange = function(){
                // ignore if non-consequence key is pressed (such as shift, ctrl, alt, escape, caps, pg up/down, home, end, arrows)
                if (last_keyCode >= 9 && last_keyCode <= 45) {
                    return;
                }
            ...
            switch(e.keyCode) {
                ...
                case 27: // Esc - deselect any active selection, hide the drop-down but stay in the field
                    ...
                    if (activeSelection > -1
                        && options.exactMatch
                        && $input_element.val() != $($('li', $results_list).get(activeSelection)).text())
                    {
                        activeSelection = -1;
                    }
    ...
    QuickSelect.matchers = {
        quicksilver : function(q,data){
            ...
            for (var i=0; i < data.length; i++) {
                ...
                if (match_label.score(match_query) > 0
                    || data[i][1] == "none"
                    || data[i][1] == "newvalue:" + data[i][0])
                {
                    self.AllItems[match_query].push(data[i]);
                }
            }
        ...
        contains : function(q,data){
            ...
            for (var i=0; i < data.length; i++) {
                match_label = (self.options.matchCase ? self.getLabel(data[i]) : self.getLabel(data[i]).toLowerCase());
                if (match_label.indexOf(match_query) > -1) {
                    self.AllItems[match_query].push(data[i]);
                }
            }
            return self.AllItems[match_query].sort(function(a,b){
        ...
        startsWith : function(q,data){
            ...
            for (var i=0; i < data.length; i++) {
                match_label = (self.options.matchCase ? self.getLabel(data[i]) : self.getLabel(data[i]).toLowerCase());
                if (match_label.indexOf(match_query) === 0) {
                    self.AllItems[match_query].push(data[i]);
                }
            }
            return self.AllItems[match_query].sort(function(a,b){
    ...
    $.fn.quickselect = function(options, data){
        if (options == 'instance' && $(this).data('quickselect'))
            return $(this).data('quickselect');
        ...
        if (!options.delay)
            options.delay     = (!options.ajax ? 400 : 10);
        ...
        if (options.autoFill === undefined || options.matchMethod != 'startsWith') {
            // if you're not using the startsWith match, it really doesn't help to autoFill.
