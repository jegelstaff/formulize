...
function permform()
{
	...
	$module_id = $xoopsModule->getVar('mid');
		...
		$sql="SELECT page_id,page_name FROM ".$xoopsDB->prefix("pageworks_pages");
	$res = $xoopsDB->query( $sql );
	if ( $res ) {
		$tab = array();
		while ( $array = $xoopsDB->fetchArray( $res ) ) {
		    $tab[$array['page_id']] = $array['page_name']." (".$array['page_id'].")";
		  }
	}
	
	$title_of_form = "Pageworks Permissions";
	$perm_name = "view";
	$perm_desc = 'Select the pages that each group is allowed to view:';
