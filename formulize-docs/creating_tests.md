...
## Working with Mutliple Windows

When creating Selenium tests that deal with multiple windows, Selenium Builder will not automatically switch windows and record the tests in the new windows. Instead, you will have to create your own commands manually for this process.

In Selenium Builder, you’ll want to add a step called “switchToWindow” which is located under Misc. You’ll need to give it the name of the window that has been opened. 

Once this is done, you’ll need to manually add the test cases for the new window as well. 

When you now want to switch back to the originally main window, you’ll once again use a “switchToWindow” step, but do not give it a name (JavaScript does not give the main window a name). This should switch to back to the main window and you can continue to write tests automatically. 