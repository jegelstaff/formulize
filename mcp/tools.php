...
Call with no arguments to get the list of topics, each with a one-line summary. Call again with \'topic\' set to one of those names to get its full reference page.',
					'inputSchema' => [
						...
						'properties' => [
							'topic' => [
								...
								'description' => 'Optional. The name of a function or formulizeDataHandler method to read the documentation for, or formulize_ui for the Formulize UI classes. Leave this out to get the list of available topics instead.'
							]
						]
					]
				];
	 ...
	 * some pages - plus the topics generated by Formulize itself, which are always available.
	 * @return array topic name => docs filename, only for files that exist; '' for a generated topic
	 */
	private function availableDocumentationTopics() {
		...
		// Formulize UI is documented in its catalog, which ships with the module, rather than in /docs
		$topics['formulize_ui'] = '';
		return $topics;
	}

	/**
	 * The Formulize UI reference, generated from the catalog that the in-app style guide and the
	 * documentation site are also made from, so it always matches this installation's formulize-ui.css.
	 * @param bool $summaryOnly true for the one-line summary used in the topic list
	 * @return string Markdown
	 */
	private function formulizeUiDocumentation($summaryOnly = false) {
		include_once XOOPS_ROOT_PATH.'/modules/formulize/include/ui_catalog.php';
		$catalog = formulize_uiCatalog();
		return $summaryOnly ? $catalog['summary'] : formulize_uiAiReference($catalog);
	}
	...
	private function get_documentation($arguments) {
		...
		if ($topic === '') {
			...
			foreach ($topics as $name => $file) {
				$list[] = [
					'name' => $name,
					'summary' => $name == 'formulize_ui'
						? $this->formulizeUiDocumentation(summaryOnly: true)
						: $this->documentationSummaryFromFile(XOOPS_ROOT_PATH.'/docs/'.$file)
				];
			}
			return [
				'topics' => $list,
		...
		if ($match == 'formulize_ui') {
			return [
				'topic' => $match,
				'documentation' => trim($this->formulizeUiDocumentation())
			];
		}
