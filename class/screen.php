	...
	function insert($screen) {
		if (!is_subclass_of($screen, 'formulizeScreen')) {
             ...
             if ($sid == 0) {
                 ...
                 $sql = sprintf("INSERT INTO %s (sid, title, fid, frid, type) VALUES (%u, %s, %u, %u, %s)", $this->db->prefix('formulize_screen'), $sid, $this->db->quoteString($title), $fid, $frid, $this->db->quoteString($type));
             } else {
                 $sql = sprintf("UPDATE %s SET title = %s, fid = %u, frid = %u, type = %s WHERE sid = %u", $this->db->prefix('formulize_screen'), $this->db->quoteString($title), $fid, $frid, $this->db->quoteString($type), $sid);
             }
