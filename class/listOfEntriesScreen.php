...
class formulizeListOfEntriesScreenHandler extends formulizeScreenHandler {
		...
		$form->addElement(new xoopsFormHidden('type', 'listOfEntries'));
                ...
                foreach($allFids as $thisFid) {
                    ...
                    for($i=0;$i<count($thisFidElements);$i++) {
                        ...
                        $elementOptions[$thisFidElements[$i]] = printSmart(trans(strip_tags($elementHeading)), 75);
                        $elementOptionsFid[$thisFid][$thisFidElements[$i]] = printSmart(trans(strip_tags($elementHeading)), 75); // for passing to custom button logic, so we know all the element options for each form in framework
                        $handleOrId = $selectedFramework ? $allHandles[$thisFidElements[$i]] : $thisFidElements[$i];
	...
	function render($screen) {
            ...
            displayEntries($formframe, $mainform, "", 0, 0, $screen);
        }
...
function addElementLOE($element, $table) {
    $table .= "<tr><td class=\"head\">" . $element->getCaption();
...
function addCustomButton($caid, $thisCustomAction, $allFids, $allFidObjs, $elementOptions, $fid) {
    ...
    if(count($allFids) > 1) {
        for($i=0;$i<count($allFids);$i++) {
            if($allFids[$i]==$fid) { continue; } // don't treat the current form as if it's an 'other' form
            $applyToOptions['new_'.$allFids[$i]] = _AM_FORMULIZE_SCREEN_LOE_CUSTOMBUTTON_APPLYTO_NEW_OTHER . printSmart($allFidObjs[$allFids[$i]]->getVar('title'), 20) . "'";
            $applyToOptions['new_per_selected_'.$allFids[$i]] = _AM_FORMULIZE_SCREEN_LOE_CUSTOMBUTTON_APPLYTO_NEW_OTHER . printSmart($allFidObjs[$allFids[$i]]->getVar('title'), 20) . _AM_FORMULIZE_SCREEN_LOE_CUSTOMBUTTON_APPLYTO_NEWPERSELECTED_OTHER;
        }
    }
...
function addCustomButtonEffect($caid, $effectid, $thisCustomAction, $allFids, $elementOptions, $applyToDefault, $fid) {
    ...
    $headerText = _AM_FORMULIZE_SCREEN_LOE_CUSTOMBUTTON_EFFECT . " $effectNumber<br /><br />" . _AM_FORMULIZE_SCREEN_LOE_CUSTOMBUTTON_EFFECT_DESC;
